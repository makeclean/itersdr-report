...
were of interest 1$\times$10$^5$ seconds, 1$\times$10$^6$ seconds and 
1$\times$10$^7$ seconds. The \gls{sdr} photon dose rate was determined in a 
single large mesh which covered the entire geometry. The sources were produced 
in seven meshes which convered discrete non-overlapping parts of the geometry. 
The use of multiple meshes based sources allows the determination of which 
source mehses are the source of resultant dose rate.
\\
\\
...
by a factor of $\sim$ 2-3, for the longest decay times by a factor of at most 
...
specific region. However, it was also found that without the contribution from
the local source, the cross talk from other systems is greater than 100 
$\mu$Sv/hr for all decay times.
\\
\\
For the \gls{upi} it was found that a significant component of \gls{sdr} is due
to the activation of structures around and include Poloidal Field Coil 2, such 
that if the activation in the \gls{upi} was reduced to zero, the dose rate would
still be several hundred $\mu$Sv/hr at 1$\times$10$^6$ seconds.
\newpage
\clearpage
\section{Purpose}
...
\end{figure}
The geometry was faceted to a tolerance of 1.0$\times$10$^4$ cm, meaning that 
the edges of curve facets used in this analysis can lie no further away from 
...
definitions were taken from the \gls{gepp} \gls{mcnp} model\cite{epp_materials}.
...
the current state of a number of software libraries. Any change to the DAG interface
is found using unit tests which are run whenever a developer wishes to merge 
in changes to the master repository. There is a defence in depth against bugs creeping
into DAG-MCNP5. There are DAGMC unit tests run often multiple time a day depending upon
pull request flow. DAGMC itself depends upon MOAB, who also have unit test run upon
opening of a pull request and also have nightly tests for a number of common system
configurations.
\\
\\
The parts of the workflow related to PyNE are also tested upon pull request by a large
number of unit tests, again often several times per day. Formal verification for indvidual
parts of PyNE are performed on an as needed basis, for example with PyNE.r2s on the FNG
...
used by \gls{ornl} as part of ADVANTG to produce weight windows for this analysis.
...
1.0$\times$10$^5$ s, 1.0$\times$10$^6$ s and 1.0$\times$10$^7$ s, approximately
...
The particle transport software used for this analysis was \gls{dag}-\gls{mcnp}5
which integrates the DAGMC toolkit into the Monte Carlo code \gls{mcnp}. 
DAG-MCNP5 has been extensively used in other ITER analyses and has a significant
validation effort behind it. MCNP5 is the standard monte carlo transport code
...
fusion relevant activation calculations. Alara has been validated against 
...
\\
\\
The individual meshtals were read using PyNE's Mesh class and statistically
