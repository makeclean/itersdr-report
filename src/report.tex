...
%% NOTE [PPHW]: Describe problems with FW-CADIS in SDR calculations
...
%% NOTE [PPHW]: Are we not able to document specific changes made to these
%% models?  I'm happy for this section to be multiple pages...
...
%%NOTE [PPHW]: How was the B4C layer added to the geometry??  Cutting out a
%%thin layer of the bioshield?  Or adding a thin layer to the bioshield?
%%Or???
...
%% NOTE [PPHW]: Doesn't yt belong in the next section?  It may be
%% validated and/or have a version number, but it is really an aux
%% code.
...
vitamin-j structure in a spatial mesh voxel that was $20 \times 20 \times 20$
cm.

%% NOTE [PPHW]: Confirm mesh voxel size.  
\\
\\
...
in excess of 12 Gb per process. It was therefore determined that in order to perform
...
performed using the \texttt{rand hist=n} command in \gls{mcnp5}.
\begin{figure}[ht!]
  \centering
...
This job-parallel approach can also be important to mitigate the impact of
pathologically long histories often experienced in problems with extreme
variance reduction.  In such problems, individual histories may experience the
variance reduction scheme in a way that results in extreme splitting of the
history.  Such rare histories can overwhelm the total computer time needed for
a simulation appearing to never actually reach completion, with severe
negative impacts on load balancing under a typical message passing parallel
approach.  In the job parallel approach, it is possible to submit many jobs
such that the total number of histories across the entire campaign is larger
than required, and use a subset of those statistcally independent jobs to
produce the final result.

%% NOTE [PPHW]: Should integrate this discussion (previously in the Conclusion
%% section) with the above.  But did we use CTME or NPS for the HTC calcs?
%% It must have been NPS to avoid overlapping histories.
...
%% NOTE [PPHW]: is ORNL happy with this description?  How is the version of
%% Denovo distributed with AVANTG related to SCALE?
...
%% NOTE [PPHW]: What does "Fractional Strength" mean?
\begin{table}[ht!]
   \begin{tabular}{| l | c |}
      \hline 
...
dose rate.
%% NOTE [PPHW]: "longer fluence operation" than what??  
\\
\\
...
%% NOTE [PPHW]: Where is the ray effect??? I don't see it.
\\
\\
...
%% NOTE [PPHW]: Let's reframe the following in a more constructive way. We
%% should just declare that we used the first N jobs that completed their
%% requested M histories.  It should come here, before reportin results (or
%% earlier in methodology, perhaps).
...
%% NOTES [PPHW]: We need some discussion here - at least to describe what is
%% in these plots in text an refer to the appendix for the others.

The total photon source intensities for each decay time are shown in
figure \ref{fig:source_dc1_no4bc} through \ref{fig:source_dc3_no4bc}.
...
\subsection{Conclusion}
The shutdown photon source produced case and 3 decay times results in
