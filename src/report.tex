...
problem.  The final geometry contained VVV volumes bounded by SSS surfaces.
\\
\\
...
In the radial direction, a cylindrical region is used to define a vacuum
boundary condition.   %% Check this
...
geometry, such as the XXXXXXXXX of the vacuum vessel and the bioshield. 
...
by examining several lineouts through the data. Specifically we can examine
...
\clearpage
\subsection{Comparison}
%Given the Figures in the previous section and the differences in the
%neutron transport section, direct comparison of the photon sources is
%better performed on a macroscopic scale. The total photon sources produced
%by the activation calculations for both models and all decay times are shown
%in Table \ref{tab:source_norms}.
%\begin{centering}
% \begin{table}[ht!]
% \centering
%  \begin{tabular}{| c |  c |  c | c |}
...
%Overall it can be seen the the photon source normalizations are consistently higher 
%for the baseline case than the B$_4$C case. There some differences seen 
%where the B$_4$C case can be higher, this is typically in mesh one, where 
%the variance reduction is trying to reduced the neutron population therefore its likely
%that this source result is dominated by noise, confirmed by comparison of Figure 
%\ref{fig:source_dc1_no4bc_b} and \ref{fig:source_dc1_b4c}. Comparison of source normalisations
%for mesh 4 shows equivalent source normalizations at all decay times.
%\\
%\\
...
There are slight differences between the baseline and B$_4$C cases, due mostly 
to the different magnitudes of the spectra, these differences are likely due to 
slight differences in the neutron transport in these two different cases. At 
decay time one, the source densities are $4.824\times10^7$ and 
$3.517\times10^7$ for the baseline and with B$_4$C cases respectively, and for 
decay time three are  $4.340\times10^6$ and $3.000\times10^6$
respectively, showing small difference with the B$_4$C case.
...
there are significant differences between the baseline and B$_4$C cases, 
due mostly to the different magnitudes of the spectra, these differences are 
likely due to the impact of B$_4$C on the transport. At decay time one, the 
source densities are $2.221\times10^4$ and  $7.102\times10^3$ for the
baseline and with B$_4$C cases respectively, and for decay time three are  
$1.548\times10^4$ and $5.594\times10^3$ respectively, showing a factor of 3 
...
Similarly to the \gls{pfc}2 case the \gls{upp} flange spectra shown in Figure 
\ref{fig:src_spectra_upp}, there are slight differences between the baseline and 
B$_4$C cases, due mostly to the different magnitudes of the spectra, these 
differences are likely due to slight differences in the neutron transport in 
these two different cases. At decay time one, the source densities are 
$6.647\times10^4$ and  $4.326\times10^4$ for the baseline and with B$_4$C 
cases respectively, and for decay time three are  $2.454\times10^4$ and 
$1.794\times10^4$ respectively, showing small difference with the B$_4$C case.
...
an \ref{fig:photons_dc1_b4c_contours}, the second decay times are shown in Figures \ref{fig:photons_dc2_nob4c_contours}
an \ref{fig:photons_dc2_b4c_contours}, and the third decay times are shown in Figures \ref{fig:photons_dc3_nob4c_contours}
an \ref{fig:photons_dc3_b4c_contours}.
\subsubsection*{Decay Time 1}
\begin{figure}[ht!]
\centering
\includegraphics[trim={0cm 8cm, 0cm 8cm},clip,scale=0.4]{../plots/final_model/Photon_Dose_Rate_Contours_Upper_Decades_Decay_Time_1.png}
\caption{Contours of total dose rate for decay time 1 for the baseline case}
\label{fig:photons_dc1_nob4c_contours}
\end{figure}
\begin{figure}[ht!]
\centering
\includegraphics[trim={0cm 8cm, 0cm 8cm},clip,scale=0.4]{../plots/final_model_with_b4c/Photon_Dose_Rate_Contours_Upper_Decades_Decay_Time_1.png}
...
\begin{figure}[ht!]
\centering
\includegraphics[trim={0cm 8cm, 0cm 8cm},clip,scale=0.4]{../plots/final_model/Photon_Dose_Rate_Contours_Upper_Decades_Decay_Time_2.png}
\caption{Contours of total dose rate for decay time 2 for the baseline case}
\label{fig:photons_dc2_nob4c_contours}
\end{figure}
\begin{figure}[ht!]
\centering
\includegraphics[trim={0cm 8cm, 0cm 8cm},clip,scale=0.4]{../plots/final_model_with_b4c/Photon_Dose_Rate_Contours_Upper_Decades_Decay_Time_2.png}
...
\begin{figure}[ht!]
\centering
\includegraphics[trim={0cm 8cm, 0cm 8cm},clip,scale=0.4]{../plots/final_model/Photon_Dose_Rate_Contours_Upper_Decades_Decay_Time_3.png}
\caption{Contours of total dose rate for decay time 3 for the baseline case}
\label{fig:photons_dc3_nob4c_contours}
\end{figure}
\begin{figure}[ht!]
\centering
\includegraphics[trim={0cm 8cm, 0cm 8cm},clip,scale=0.4]{../plots/final_model_with_b4c/Photon_Dose_Rate_Contours_Upper_Decades_Decay_Time_3.png}
...
shown in Figures \ref{fig:ct_lp_dc1} to \ref{fig:ct_lp_dc3}. Both cases find the dose rate in 
