...
source. Although it is not possible to rigorously asses the uncertainty in a calculation 
...
relased with the introduction of so much B$_4$C as shielding material.
\\
\\
...
For the generation of weight windows, \gls{advantg} was used to employ the 
\gls{fwcadis} method to generate the weight windows and the source
biasing parameters for the neutron transport calculation of this analysis. 
The adjoint source in \gls{advantg} was defined to represent a rectangular 
parallel pipe that extend from x=-1000 cm to x=1900 cm, y=-1000 cm to y=1000 cm, 
and z=-1550 cm to z=1900 cm. These dimensions are much larger than the
dimensions of the 40° sector of the ITER Clite model in the x and y 
directions to account for the reflection in these directions. Denovo was
configured to run with the FENDL-2.1 46 group neutron photon library, with solver settings
using a quadruple range quadrature set \cite{jarrell} with 8 azimuthal angles and 8 polar angles,
and a P$_3$ diffusion correction was used. 
...
sector, or fills the entire 360$^{\circ}$. In terms of discretizing the geometry
30 rays were fired per element along the z-direction and material mixture
sensitivity was set such that a tolerance of 0.01 was used. The Denovo
...
The \gls{fwcadis} response weighting was set to produce a uniform variance in the problem
across all energies. The spatial treatment of the \gls{fwcadis}  adjoint source was set to
be global, which does not preferentially increase the population of the simulated
neutrons in mesh tally elements with larger volumes. 
...
errors are shown in Figures \ref{fig:neutron_baseline}
and \ref{fig:neutron_baseline_relerr}, respectively.  Flux results are given
...
statistical errors are shown in Figures \ref{fig:neutron_no4bc}
and \ref{fig:neutron_no4bc_relerr}, respectively.  As with the baseline case,
