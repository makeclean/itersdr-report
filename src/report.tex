...
source. The uncertainty in the \gls{sdr} was estimated being between a relative 
error of between 0.5 and 2.0.
\\
\\
Future analysis should focus around improving analysis 1) Methods to decrease
...
\\
\\
1 - Methods to decrease the \gls{sdr} in the person 
...
shielding internal to the strong sources, possible on improving the \gls{vv} 
filler near the \gls{pfc}2 or strong local shielding. Further B$_4$C additions 
should be considered around \gls{pfc}2 and potentially in the regions between 
the vessel extensions.  
\\
\\
2 - It was been found \cite{Khripunov2001899} that neutrons may be produced in significant
quantities following machine shutdown some $2\times10^{16}$ n/s immediately 
after shutdown, and following a trend with the decay of induced radioactivity. 
The last survey of this effect was peformed with neutron diagnostics in mind 
and the impact of these photoneutrons, it would be prudent therefore to 
consider the effect that these photoneutrons may have 
post machine shutdown and during port maintenence operations. Part of this 
survey may need to do some physics code comparison regarding the treatment of
photoneutrons.
\\
\\
3 - Boron Carbide is such an effective material at reducing shutdown for 2 main 
reasons, 1) it is very effective at absorbing thermal neutrons and 2) the carbon
 present is a reasonable moderator of neutrons. The thermal capture reactions 
on B$_4$C progress via two pathways $^{10}$B(n,$\alpha$)$^7$Li and 
$^{10}$B(n,p)$^{10}$Be. The lithium producing reaction further progresses via 
$^7$Li(n,$\alpha$)$^3$H, ultimately producing tritium. Production of tritium is 
often a concern and it should be verified that such quantities 
of B$_4$C ultimately do not offgas tritium and helium at too great a level. 
...
1 - Neutronics Benchmarks - Several neutronics benchmarks have been performed 
...
2 - Activation Data Uncertainty - Activation data has uncertainty in the reaction channel encapsulated 
as part of it, often as part of activation calculations the inventory code estimate in the uncertainty 
in the nuclide due to all the formation channels, this is in the range 10-100\% depending upon the nuclide, 
we can ascribe a lower bound estimate of 10\% and 20\% for an upper bound on the basis of the nuclides in this problem
3 - Mesh Resolution - There has been only one study showing the effect of the activation mesh upon the final 
...
4 - Neutron Histories - A target uncertainty in the total neutron flux for this calculation was 
...
\\
\\
The smallest the total relative error could be therefore 0.45 and the highest reasonable estimate is 2.0
