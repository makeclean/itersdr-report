...
\newacronym{cts}{CTS}{Cryostat Thermal Shield}
...
\newacronym{vvts}{VVTS}{Vacuum Vessel Thermal Shield}
...
\\
\\
The activation results showed that there are substantial reductions in the photon
source intensity at the bioshield for all decay times due to the B$_4$C.  However
, this reduction is diminished for components further from the bioshield wall.
\\
\\
...
contain prompt photon doses which are not present following machine shutdown. It
does not contain neutrons that are the product of decay reactions, such as neutron
production from activated water, which is a small component compared to the plasma
source. Although it is not possible to rigorously assess the uncertainty in a calculation 
of this scope, the absolute relative uncertainty has been estimated as a factor of 2. That is, 
the actual independent results are expected to be between 0.5 times and 2 times the simulated results.
This estimate includes uncertainty in neutron transport, activation, photon transport and
those due to the resolution of the activation mesh. However, many of the uncertainties are
complementary, and therefore to compare the difference between the two different cases the
true relative error in the ratio would be much smaller than a factor of two, largely
dominated by the uncertainty in activation and neutron transport, therefore an upper bound
of 40\% can be ascribed and a lower bound being 10\%.
\\
\\
Future analysis should focus on improvements such as 1) methods to decrease
the \gls{sdr} 2)  consider other 
sources of radiation at shutdown, and 3) estimate the quantity of tritium 
released with the introduction of so much B$_4$C as shielding material, tritium
being produced via the $^{10}$B(n,$\alpha$)$^7$Li(n,$^3$He)$^3$H reaction.
\\
\\
Focus should be placed on methods to decrease the \gls{sdr} in locations
of planned human access, such as the equatorial and upper port interspaces. A 
significant finding in the upper ports was that a significant fraction of the
\gls{sdr} comes from decay photons created outside the upper port, from the 
activation of the structural components nearby. 
...
\\
\\
It was found that for the shortest decay time the impact of the B$_4$C layer was 
largest: at the shortest decay time the dose rate in the \gls{epi} was reduced
...
%\renewcommand*{\theHsubsection}{\the\value\one{subsection}}
%\renewcommand*{\theHsubsubsection}{\the\value\two{subsubsection}}

\section{Purpose}
\subsection{Problem Statement}
The shutdown dose rate in an around the equatorial and upper ports determines the
type and duration of maintenance that can be performed by personnel access. Thus,
minimization of the dose rate is desirable and indeed encouraged by the 
\gls{alara_p} principle used at ITER. Since initial estimates indicate that
these dose rates are larger than desirable, it was suggested in the 
...
The \gls{r2s} method \cite{r2s} is one method of generating shutdown
...
so on. The root cause of long histories is the inconsistency of the materials
definitions between the deterministic and the Monte Carlo models in the hybrid
MC/deterministic method implementation. Even with the materials mixing
implemented in \gls{advantg} \cite{advantg}, the accurate representation of the
materials of the Monte Carlo model using a Cartesian deterministic mesh is
practically impossible. Consequently the variance reduction parameters
generated using the FW-CADIS method may suffer from inaccuracies, especially
in narrow and complicated streaming paths, that can decrease the efficiency of
the Monte Carlo calculation.  
Single core simulation long histories are not so much of a problem as at some
point the calculation will terminate. However, in large scale MPI-based parallel
...
queuing system. In this work we mitigate some the the problems with the
parallel version of MCNP by using a distributed as described in
\ref{section:job_level_parallelism}.
...
in and whether it was modified as part of this analysis or not. Where possible mass
was conserved, however for very small components for example when filling bolt holes
small bolt heads and threads were discarded.
...
\begin{centering}
 \begin{table}[ht!]
  ...
  Poloidal Field Coils & CLITE\_V1\_REV131031\_SPACECLAIM & SpaceClaim & N\\
...
New volumes, referred to as Graveyard volumes,  were created in order to apply 
...
Due to the necessity of model simplification in the process of generating native MCNP 
...
the in-wall plates within the vacuum vessel shells, where the in-wall plates were
unmodified from how they were delivered by US-ITER. Also, overlaps were removed 
...
The analysis codes used in these calculations are shown in Table 
...
\gls{advantg}, specifically using a version of \gls{advantg} coupled to \gls{dagmc}
, which allows a
...
by the geometry or weight window map, but allows the fidelity of the tally
...
fewer ``wasted'' mesh voxels outside the geometry for which there will be no
answer, allowing for further memory reductions or fidelity increases.


\subsubsection{Job-level Parallelism}
\label{section:job_level_parallelism}
...
launched for that specific mesh tally but the random number seed is strided in
...
equivalent to $1.97 \times 10^{19}$ neutrons per second in the 40$^{\circ}$
...
$10^5$ s, $10^6$ s and $10^7$ s, approximately corresponding to 28 
hours, 11.6 days, and 116 days post shutdown respectively.
...
\begin{centering}
 \begin{longtable}[ht!]{ p{0.2\textwidth} | p{0.3\textwidth} | p{0.3\textwidth} }
  ...
  M101  & 316L(N)-IG &  CLITE V1 \\
  M102  & SS304LN  &  CLITE V1 \\
  M103  & SS304/304L &  CLITE V1 \\
  M104  & SS304 &  CLITE V1 \\
  M105  & SS304L &  CLITE V1 \\
  M106  & SS316L &  CLITE V1 \\
  M107  & XM-19      &  CLITE V1 \\
  M108  & SS316L &  CLITE V1 \\
  M109  & STEEL 660 &  CLITE V1 \\
  M110  & SS316LN &  CLITE V1 \\
  M111  & SS316L &  CLITE V1 \\
  M161  & H$_2$0 + 316L(N)-IG + XM19 + SS304B4 &  CLITE V1 \\
  M170  & H$_2$0 + 316L(N)-IG &  CLITE V1 \\
  M200  & Concrete    &  CLITE V1 \\
  M303  & NiAl Bronze &  CLITE V1 \\
  M400  & H$_2$O &  CLITE V1 \\
  M601  & H$_2$O + 316L(N)-IG &  CLITE V1 \\
  M602  & H$_2$O + 316L(N)-IG &  CLITE V1 \\
  M603  & H$_2$O + CuCrZr-IG  &  CLITE V1 \\
  M611  & H$_2$O + 316L(N)-IG &  CLITE V1 \\
  M613  & H$_2$O + CuCrZr-IG &  CLITE V1 \\
  M621  & H$_2$O + 316L(N)-IG &  CLITE V1 \\
  M622  & H$_2$O + 316L(N)-IG &  CLITE V1 \\
  M623  & H$_2$O + 316L(N)-IG &  CLITE V1 \\
  M631  & H$_2$O + CuCrZr-IG &  CLITE V1 \\
  M906  & Magnet Conductor &  CLITE V1 \\
  M907  & TFC Outboard &  CLITE V1 \\
  M908  & TFC WP &  CLITE V1 \\
  M910 & B$_4$C & Bespoke  \\
  M911 & Al T6061 & Bespoke  \\
  M912 & Al T6061 & Bespoke  \\
  M913 & (Copy M100 - Clite V1) 316L(N)-IG & Bespoke\\
  M914 & (Copy M100 - Clite V1) 316L(N)-IG & Bespoke\\
  M915 & Copy M910 & Bespoke \\
  M916 & (Copy M100 - Clite V1) 316L(N)-IG & Bespoke\\
  M917 & Cryopipes & \cite{cryopump_communication}\\
  M918 & 316L(N)-IG & CLITE V1 \\
  M920 & (Copy M100 - Clite V1) 316L(N)-IG & Bespoke\\
  M921 & (Copy M100 - Clite V1) 316L(N)-IG & Bespoke\\
  M922 & (Copy M100 - Clite V1) 316L(N)-IG & Bespoke\\
  M923 & (Copy M100 - Clite V1) 316L(N)-IG & Bespoke\\
  M924 & (Copy M100 - Clite V1) 316L(N)-IG & Bespoke\\
  M925 & (Copy M100 - Clite V1) 316L(N)-IG & Bespoke\\
  M926 & 316L(N)-IG - Frame Wheels & \cite{bertalot_communication}\\
  M927 & 316L(N)-IG - Frame Wheels Drive & \cite{bertalot_communication}\\
  M928 & 316L(N)-IG - Port Plug Frame & \cite{bertalot_communication}\\
  M929 & 316L(N)-IG - Port Plug Pipes & \cite{bertalot_communication}\\
  M930 & 316L(N)-IG & \cite{cryopump_communication}\\
  M931 & 316L(N)-IG:0.7 H$_2$O:0.3 & \cite{bertalot_communication}\\
  M932 & 316L(N)-IG:0.9 H$_2$O:0.1 & \cite{bertalot_communication}\\
  M933 & 316L(N)-IG:0.9 H$_2$O:0.1 & \cite{bertalot_communication}\\
  M934 & 316L(N)-IG:0.97 H$_2$O:0.03 & \cite{bertalot_communication}\\
  M937 & 316L(N)-IG & \cite{cryopump_communication}\\
  M938 & 316L(N)-IG & \cite{cryopump_communication}\\
  M939 & 316L(N)-IG & \cite{cryopump_communication}\\
  M935 & First Wall & BL-lite \\
  M936 & First Wall & BL-lite \\
  M940 & 316L(N)-IG & Diagnostics MCNP Model \\
  M941 & 316L(N)-IG & Diagnostics MCNP Model \\
  M942 & 316L(N)-IG & Diagnostics MCNP Model \\
  M943 & 316L(N)-IG & Diagnostics MCNP Model \\
...
\\
\\
Activation Data - The activation data used was EAF-2010.
\\
\\
...
\gls{advantg} was used to employ the \gls{fwcadis} method to generate the 
weight windows and the source biasing parameters for the neutron transport
calculation of this analysis. The adjoint source in \gls{advantg} was defined
to represent a rectangular right parallelepiped that extend from x=-1000 cm to x=1900
cm, y=-1000 cm to y=1000 cm, and z=-1550 cm to z=1900 cm. These dimensions are
much larger than the dimensions of the 40$^{\circ}$ sector of the ITER Clite
model in the x and y directions to account for the reflection in these
directions. The Denovo mesh was divided in the x direction from -1800.0 to 0.0
cm into 84 bins and from 0.0 to 1800 cm into 180 bins. The solution mesh was
split in the y direction from - 1800.0 to -650.0 into 45 bins, from -650.0 to
650.0 cm into 130 bins, and from 650.0 to 1800.0 cm into 45 bins. The solution
mesh was split in the z direction from -1500.0 to 1900.0 cm into 240 bins. The
differing resolutions used in the mesh was to focus the detailed parts of the
mesh into the 40$^{\circ}$ sector of interest, using the rest of the lower
resolution parts to ensure the correct boundary conditions. To optimize the
population of the simulated neutrons in each of the individual neutron energy
group in the Monte Carlo calculation, the \gls{advantg} response weighting
option (fwcadis\_response\_weighting) was set to be false. When this option is
true, \gls{advantg} constructs the adjoint source to accelerate an
energy-integrated response (e.g., dose or total flux). When it is
False, \gls{advantg} generates parameters with the objective of obtaining
relatively uniform uncertainties across all energy groups. The spatial
treatment of the \gls{fwcadis} adjoint source weighting
(fwcadis\_spatial\_treatment) was set to be “global” as recommended for mesh
tallies \cite{advantg}. This treatment does not preferentially increase the
population of the simulated neutrons in mesh tally elements with larger
volumes. A Quadruple Range Quadrature \cite{jarrell} with 8
polar angles and 8 azimuthal angles was used with a P 3 scattering order for
the Denovo forward and adjoint calculations in this analysis. The Denovo
calculations used a FENDL-2.1 46 group neutron photon library.
...
\begin{figure}[ht!]
  ...
  \caption{The seven different mesh domains, (left) shown with some transparency
           ...
           absolute size [cm] of the mesh regions relative to the entire ITER geometry.}
...
left hand image of Figure \ref{fig:wwinp_thermal}. The overall span of the weight window
...
the attenuation in the range of 6 to 8 orders of magnitude. Overall the solution appears reasonable.
\\
\\
...
rotation feature for \gls{advantg} that allows rotationally symmetric
bodies.  Therefore, weight window values are observed beyond the 40$^{\circ}$ sector.

\subsection{Baseline Neutron Flux Results}

For the baseline case, the total neutron flux and its associated statistical
errors are shown in Figures \ref{fig:neutrons_baseline}
and \ref{fig:neutrons_baseline_relerr}, respectively.  Flux results are given
in units of neutrons/cm$^2$/src, and are converted to fluxes by normalizing to
the neutron source intensity for each pulse in the irradiation schedule as
shown in column 3 of Table \ref{tab:irrad_scenario}.
...
the \gls{upi} \& \gls{epi}, and the divertor pumping port contains only a
cryopump. The open pumping port specifically leads to higher neutron fluxes
beyond the vacuum vessel.
...
\begin{figure}[ht!]
  ...
  \caption{The total neutron flux from the baseline case $[n/cm^2/src]$}
...
acceptable for an activation calculation.  This limit was deemed acceptable for
several reasons ; most of the dose comes from the first neutron activation path
for a given daughter hence uncertainty strongly correlates with the error in total flux and
the final estimate of uncertainty includes this error. It is however on the cusp of being
acceptable and future work should aim to further reduce this error to less than 10\%.
The challenges with producing an acceptable 
...
For the case with B$_4$C liner, the total neutron flux and its associated
statistical errors are shown in Figures \ref{fig:neutrons_no4bc}
and \ref{fig:neutrons_no4bc_relerr}, respectively.  As with the baseline case,
flux results are given in units of neutrons/cm$^2$/src, and are converted to
fluxes by normalizing to the neutron source intensity for each pulse in the
irradiation schedule as shown in column 3 of Table \ref{tab:irrad_scenario}.

Relative to the baseline model, these results include only the addition 
of the B$_4$C liner.  As with the baseline model, relative to the standard 
C-lite native \gls{mcnp5} model there is much more
...
\begin{figure}[ht!]
  ...
  \caption{The total neutron flux from the B$_4$C case $[n/cm^2/src]$}
...
The statistical uncertainties in the neutron fluxes for the case with the
B$_4$C liner are nominally the same as for the case without: $\sim$ 10-20\% in
the locations where the activation is likely to contribute to the dose.
...
\begin{figure}[ht!]
  ...
  \caption{The total flux and flux ratio along a line 0,0,0 cm to 1800,0,0 cm}
...
\begin{figure}[ht!]
  ...
  \caption{The total flux and flux ratio along a line 0,0,500 cm to 1800,0,500 cm}
...
however in some cases it is hard to determine what is the true effect due to the high 
...
neutrons there as they will not contribute outside the vacuum vessel. However, in the 
...
the B$_4$C layer is effective at absorbing thermalized neutrons in the \gls{epi} and \gls{upi} and
that the bioshield the source of the thermal neutrons given that the thermal flux build back up
immediately after the B$_4$C layer.
...
merged together properly. These geometry deficiencies result in a small number histories that are
effectively lost without being detected as lost particles. They occur at a location in the
geometry that should not impact the most important results in the port interspace regions.
\newpage
\clearpage
...
\clearpage
\subsection{Comparison}
As describe Section \ref{sec:phtn_trans_method}, the source from each mesh
region is transported separately, and the resultant photon dose maps are added
in superposition to determine the total photon dose map.  This requires
appropriate normalization of each result based on the total photon source
intensity of each mesh region.  These are shown for each case and each decay
time in Table \ref{tab:source_norms}.
...
fluctuations in the neutron transport.  This is reinforced by the very small
...
largest activation due to that larger neutron flux.
...
This section shows the shutdown photon dose rate data that was performed using
the photon sources determined in the previous section. Figures \ref{fig:photons_dc1_no4bc_total}
to \ref{fig:photons_dc3_no4bc_total_error_zoomed} show the \gls{sdr} in the baseline case 
...
\begin{figure}[ht!]
\centering
\includegraphics[trim={0cm 8cm, 0cm 8cm},clip,scale=0.51]{../plots/final_model/Photon_Dose_Rate_Contours_Upper_Decades_Decay_Time_1.png}
\caption{Contours of total dose rate for decay time 1 for the baseline case, contours at
1.0, 1.5, 2.0, 2.5, and 3.0 mSv/hr}
\label{fig:photons_dc1_nob4c_contours}
\end{figure}
\begin{figure}[ht!]
\centering
\includegraphics[trim={0cm 8cm, 0cm 8cm},clip,scale=0.51]{../plots/final_model_with_b4c/Photon_Dose_Rate_Contours_Upper_Decades_Decay_Time_1.png}
\caption{Contours of total dose rate for decay time 1 for the B$_4$C case, contours at
        1.0, 1.5, 2.0, 2.5, and 3.0 mSv/hr}
...
\begin{figure}[ht!]
\centering
\includegraphics[trim={0cm 8cm, 0cm 8cm},clip,scale=0.51]{../plots/final_model/Photon_Dose_Rate_Contours_Upper_Decades_Decay_Time_2.png}
\caption{Contours of total dose rate for decay time 2 for the baseline case, contours at
        1.0, 1.5, 2.0, 2.5, and 3.0 mSv/hr}
\label{fig:photons_dc2_nob4c_contours}
\end{figure}
\begin{figure}[ht!]
\centering
\includegraphics[trim={0cm 8cm, 0cm 8cm},clip,scale=0.51]{../plots/final_model_with_b4c/Photon_Dose_Rate_Contours_Upper_Decades_Decay_Time_2.png}
\caption{Contours of total dose rate for decay time 2 for the B$_4$C case, contours at
        1.0, 1.5, 2.0, 2.5, and 3.0 mSv/hr}
...
\begin{figure}[ht!]
\centering
\includegraphics[trim={0cm 8cm, 0cm 8cm},clip,scale=0.51]{../plots/final_model/Photon_Dose_Rate_Contours_Upper_Decades_Decay_Time_3.png}
\caption{Contours of total dose rate for decay time 3 for the baseline case, contours at
        1.0, 1.5, 2.0, 2.5, and 3.0 mSv/hr}
\label{fig:photons_dc3_nob4c_contours}
\end{figure}
\begin{figure}[ht!]
\centering
\includegraphics[trim={0cm 8cm, 0cm 8cm},clip,scale=0.51]{../plots/final_model_with_b4c/Photon_Dose_Rate_Contours_Upper_Decades_Decay_Time_3.png}
\caption{Contours of total dose rate for decay time 3 for the B$_4$C case, contours at
        1.0, 1.5, 2.0, 2.5, and 3.0 mSv/hr}
...
$10^{10}$ $\mu$Sv/hr in the plasma chamber to $10$'s of $mu$Sv/hr near the bioshield, the data are necessarily shown on a log scale, 
...
vessel are unaffected by the B$_4$C layer.  Note that the high statistical uncertainty
from the photon transport means that no conclusions can be made about the
relative dose inside the equatorial port plug, however the internals of this
region are not of primary interest in this study.
...
activation of steel components near to the bioshield, due to the impact of the 
B$_4$C layer. 
\\
\\
...
beyond the thermal shields are somewhat shielded by it. The \gls{bs} is also a
significant source, unshielded due to is proximity to the \gls{epi}. Photon cross 
talk is also present, the \gls{lp} being unplugged leads
...
\begin{itemize}
\item{Geometry simplification - the ITER CAD geometry model is composed of several 
      independently constructed models of individual parts of the geometry. Several stages of 
      CAD defeaturing takes place where extraneous details are removed. Each complex
      CAD surface is simplified to an analytic surface which approximates the true CAD surface.
      This process necessarily generates a degree of approximation.} 
\item{Geometry uncertainty- there is an uncertainty associated with geometry. In DAGMC
      the underlying CAD geometry is faceted to within a given tolerance, such that the
      triangular facets that the geometry is composed of lie within a given delta of the CAD
      surface. }
\item{Material composition - Material compositions have uncertainties due to deviations from
      the materials as the device would be built. Often materials are defined by the combination
      of several other materials and are determined by homogenization, which leads to further
      uncertainty.}
\item{Transport nuclear data uncertainty - there is an uncertainty associated with each 
      reaction channel in each cross section in the problem for both photons and neutrons.}
\item{Neutron mesh resolution - the bounds of the neutron mesh define the 
      profile of each volume element.  This introduces two separate kinds of uncertainty.
      First, how large the mesh element is relative to the neutron mean free paths defines 
      how well this mesh captures gradients in the total neutron flux and/or the neutron 
      flux spectrum.  This mesh is used to define homogenized material 
      compositions for the activation calculation.  When combined with the uncertainty in
      the ability to capture gradients, this can results in a simulation that exposes materials
      that occur in locations at the high end of the gradient to fluxes that are too low, and 
      exposes materials that occur at the low end of the gradient to fluxes that are too high.
      Finally, this mesh is also used for the photon source.  Although care is taken to avoid
      sampling the source from regions of space that contain vacuum, it is possible that a
      photon that could only be born in one part of a mesh voxel is sampled from another part
      of a mesh voxel.  Understanding the implications of this uncertainty is an ongoing
      research effort and may result in substantial systematic error.}
...
\item{Neutron transport uncertainty - the neutron transport calculation has intrinsic uncertainties
     in the calculation. These uncertainties are linked to how low the statistical error is 
     in the calculation, but also how converged the neutron spectra in a given mesh element is. For instance
     the uncertainty in a given volume element may have a very low error in the total bin, but 
     a large error in the thermal bins, when propagated through to an activation calculation
     the activation may be dominated by the thermal bin and thus the estimate of the photon source 
     in that volume element would be underestimate or overestimated.}
\end{itemize}

There does not exist a way to formally estimate the uncertainty for all of
these different contributions, nor is there a mechanism to combine them
mathematically even if they were all well understood.  Estimates of these
quantities are the source of active research at a number of institutions world
wide.  It may be useful, however, to rank them based on there expected
contribution to the overall uncertainty in results.  The largest source of
uncertainty in the final result is expected to be due to the neutron mesh
resolution, followed by stochastic uncertainties in the neutron transport and
undersampling.  Nuclear data uncertainties and uncertainty due to geometry
simplification and material composition are expected to be substantially
smaller.  Uncertainty due to DAGMC handling of geometry has been shown to be
extremely small.\cite{snouffer}
\\
\\
We can however attempt to estimate and lower bound and a reasonable upper bound. Estimation of the uncertainty 
in the SDR is challenging due to a number of sources of uncertainty which may compensate each other in unexpected 
ways. Although no formal method to determine the uncertainty in an SDR calculation exists, an
estimate is provided below.

\begin{enumerate}
\item{Neutronics Benchmarks - Several neutronics benchmarks have been performed 
\cite{Kodeli2004717,Batistoni19941724,Fausser20112135,Batistoni1995504,Batistoni1997233,Flammini20151964,Klix2016,Kodeli2016,Angelone2016}
and the majority show deviations from the experimental result of at between 5-50\%, we therefore 
can ascribe a lower bound uncertainty of 5\% and an upper reasonable bound of 20\% in the neutron 
result purely from nuclear data}
\item{Activation Data Uncertainty - Activation data has uncertainty in the reaction channel encapsulated 
as part of it. Inventory codes are often able to provide an estimate in the uncertainty 
of the production of the nuclide due to all the formation channels, this is nuclide dependent but typically in the range 10-100\%. 
We can ascribe a lower bound estimate of 10\% and 20\% for an upper bound on the basis of the nuclides in this problem}
\item{Mesh Resolution - There has been only one study showing the effect of the activation mesh upon the final 
result in an SDR calculation \cite{r2smesh}, and for cell based methods \cite{frej}, these suggest that the mesh may have 
an uncertainty of 10\% at its lowest and an upper bound of 40\%}
\item{Neutron Histories - A target uncertainty in the total neutron flux for this calculation was 
10\%, however this does not ascribe any uncertainty limits to the neutron spectrum, a reasonable 
estimate of the effect is difficult to summarize, but a relative error of 10\% directly form the 
flux as a lower bound and an upper bound of 20\% is ascribed.}
\end{enumerate}

Based on this information combined with expert judgment, the most optimistic
estimate of the error suggests that the true results are between 0.7 and 1.45
times the reported results.  A more pessimistic estimate of the error suggests
that the true results are between 0.5 and 2.0 times the reported results.
Since some of the largest sources of error in this problem are systematic, the
ratio between the results that include the B$_4$C liner and those that do not
are expected to have much smaller errors, between 10\% and 40\%.
...
\subsection*{ Transport Improvements}
\subsubsection*{Further Neutron Histories}
This calculation ran a specific number of neutron histories which was limited by
time constraints. Future calculations should run a greater number of neutron histories
which would reduce the statistical noise in the \gls{sdr} photon sources and allow
smaller uncertainty bars to be placed on the photon sources. 

\subsubsection*{Higher Resolution \gls{sdr} Mesh}
The neutron mesh decomposition used in this calculation was specifically chosen
...
complexity increases as more neutron transport calculations must be performed. 


\subsection*{Variance Reduction}

In addition to computational inefficiency caused by the flaws in the
final \gls{cad} geometry used for neutron transport, this work was impacted by 
long histories.  This appear to be a pervasive problem in the ITER geometry due 
to the combination of streaming paths and strong neutron attenuation though
...
In future calculations if a specific target region has been identified, it
...
parameters to split particles appropriately by attenuation, the combination
