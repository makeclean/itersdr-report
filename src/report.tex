...
so on. The root cause of long histories root cause is a poor description of 
geometric fidelity of the model which is impossible on a Cartesian mesh as 
elements are unlikely to correspond well to the underlying 
geometry, which leads 1) the issue that rays in the deterministic problem do not
allign with streaming channels in the problem and 2) the issue of material mixing
due to poor correlation of mesh elements and streaming channels. These inaccuracies
can decreate the efficiency of Monte Carlo simulations.
Single core simulation long histories are not so much of a problem as 
...
queuing system. In this work we mitigate some the the problems with the parallel 
version of MCNP by using a distributed as described in 
...
evidence of component shadowing, a ray effect like trace in the \gls{epi}, 
in the weight window solution in the right hand image
of Figure \ref{fig:wwinp_thermal} due to the neutron attenuation of some \gls{epi}
shielding around the diagnostic equipment. Overall the solution appears reasonable.
\\
\\
...
the \gls{upi} \& \gls{epi}, and the divertor pumping port contains only a cryopump. The
