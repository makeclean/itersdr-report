...
if [ $4 == "flux" ] ; then
    echo '\caption{The total photon dose from the baseline case for decay time '$2' for mesh '$3'}'
else
    echo '\caption{The error in photon dose from the baseline case for decay time '$2' for mesh '$3'}'
fi
echo "\end{figure}"
}
